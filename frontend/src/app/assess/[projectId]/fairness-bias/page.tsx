        ...
        <div className="p-5 space-y-6">
          ...
          <div className="space-y-2 py-1">
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground font-medium pt-0.5">
              ...
              <span className="font-mono">{totalQuestions - answeredQuestions} left</span>
            </div>
          </div>

          {/* Categories & Questions Tree (Flat, Un-carded layout) */}
                  ...
                  <div key={category.id} className="select-none space-y-1">
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        <ChevronRight className={`w-3.5 h-3.5 text-muted-foreground transition-transform duration-200 shrink-0 ${isExpanded ? "rotate-90" : ""}`} />
                        <CategoryIcon className={`w-4 h-4 shrink-0 ${isCurrentCategory ? "text-primary" : "text-muted-foreground"}`} />
                        <h3 className="text-xs font-semibold truncate flex-1">
                          {category.label}
                        </h3>
                      </div>
                      <span className={`ml-2 text-[10px] font-mono font-medium px-1.5 py-0.5 rounded shrink-0 ${isCatComplete ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : answeredInCat > 0 ? "bg-amber-500/10 text-amber-600 dark:text-amber-400" : "text-muted-foreground/60"}`}>
                        {answeredInCat}/{category.totalPrompts}
                      </span>
                    </div>
                    ...
                    <AnimatePresence initial={false}>
                      {isExpanded && (
                        <motion.div
                          ...
                          className="ml-3 pl-3 border-l border-border/50 space-y-0.5 pt-0.5"
                        >
                          {category.prompts.map((prompt, promptIdx) => {
                            const isCurrentQuestion =
                              isCurrentCategory && currentPromptIndex === promptIdx;
                            const isAnswered = getQuestionStatus(catIdx, promptIdx);
          ...
          <div className="w-full">
            ...
            {currentPrompt && (
              <motion.div
                ...
                className="bg-card border border-border rounded-xl p-6 md:p-8 shadow-xs space-y-6 mb-8"
              >
                <div className="border-b border-border/60 pb-4">
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-primary bg-primary/10 px-2 py-0.5 rounded border border-primary/20">
                    Prompt #{currentQuestionOrdinal} of {totalQuestions}
                  </span>
                  <h2 className="text-base sm:text-lg font-semibold text-foreground leading-relaxed mt-2.5">
                    {currentPrompt.text}
                  </h2>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {(() => {
                      ...
                      return (
                        <span className={`text-[11px] font-mono px-2 py-0.5 rounded-md transition-colors ${
                          isLimitReached 
                            ? "text-rose-500 bg-rose-500/10 font-bold border border-rose-500/20" 
                            : isNearLimit 
                  ...
                  <textarea
                    id="responseTextarea"
                    rows={8}
                    className="w-full rounded-lg border border-input bg-background p-4 text-xs font-mono leading-relaxed text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors resize-none placeholder:text-muted-foreground/60"
                    ...
                    placeholder="Type or paste your AI system prompt response here..."
                  />
                  <div className="text-xs text-amber-600 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg flex items-start gap-2">
                    <span className="shrink-0 font-bold">⚠️</span>
                    <div className="leading-relaxed">
                      <strong>Security:</strong> Your notes are automatically sanitized to prevent malicious content. HTML tags and scripts will be stripped upon evaluation.
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <Button
                ...
                variant="outline"
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-xs font-semibold border border-input text-foreground hover:bg-muted bg-transparent"
              >
              ...
              {hasNext ? (
                <Button
                  onClick={handleNext}
                  className="flex items-center gap-2 px-6 py-2.5 rounded-lg text-xs font-semibold bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm"
                >
                  Next
